...
void Road::populate_traffic() {
	...
	int start_s = max(camera_center - (update_width/2), 0);
	for (int l = 0; l < num_lanes; l++)
	{
		int lane_speed = lane_speeds[l];
		bool vehicle_just_added = false;
		for(int s = start_s; s < start_s+update_width; s++)
		{
...
void Road::add_ego(int lane_num, int s, vector<int> config_data) {
    ...
    Vehicle ego = Vehicle(lane_num, s, lane_speeds[lane_num], 0);
...
void Road::display(int timestep) {
    ...
    Vehicle ego = vehicles.find(ego_key)->second;
    int s = ego.s;
    string state = ego.state;

    camera_center = max(s, update_width/2);
    int s_min = max(camera_center - update_width/2, 0);
    int s_max = s_min + update_width;
